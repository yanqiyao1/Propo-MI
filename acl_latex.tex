...
How Large Language Models (LLMs) perform logical reasoning internally remains poorly understood: prior mechanistic studies mostly chase task-specific circuits, leaving open \emph{how} LLMs organize propositional reasoning as a computation. We address this gap with a causal mechanistic analysis on \textit{PropLogic-MI}, a controlled benchmark of 11 propositional rule categories across one- and two-hop tasks, applied to four models from three families (\textit{Qwen3}, \textit{Llama-3.1}, \textit{Mistral}). Rather than asking which components are necessary, we ask how the reasoning process is organized, and identify four interlocking mechanisms that recur across all four models: \textit{Staged Computation}, where early, middle, and late layers take on distinct functional roles; \textit{Information Transmission}, where semantic content aggregates at boundary tokens; \textit{Fact Retrospection}, in which factual information is persistently re-accessed across network depth; and \textit{Specialized Attention Heads} that implement the macroscopic patterns. A prompt-order intervention that moves the query before the facts confirms that this organization is model-internal rather than an artifact of surface layout. Together, these findings provide cross-family mechanistic evidence that propositional reasoning in LLMs is structured rather than monolithic.
...
First, we shift analytical focus from model-specific circuit identification to generalizable mechanistic principles. Rather than asking \textit{``which heads are necessary for this specific task,''} we ask \textit{``what computational strategies does the model employ to solve logical problems in general?''} As shown in Figure~\ref{fig:overview}, this perspective reveals four interlocking mechanisms: (1) \textit{Staged Computation}, where early, middle, and late layers assume distinct functional roles; (2) \textit{Information Transmission}, where semantic information aggregates at boundary tokens; (3) \textit{Fact Retrospection}, in which factual information is persistently re-accessed across network depth rather than encoded once in early layers; and (4) \textit{Specialized Attention Heads}, where heads exhibit stable functional specialization into Splitting, Transmission, and Fact-Retrieval roles.
%
Second, we expand the scope of analysis to 11 propositional logic rule categories across one-hop and two-hop reasoning, and replicate the above four mechanisms on four models from three families: \textit{Qwen3-8B}, \textit{Qwen3-14B}, \textit{Llama-3.1-8B-Instruct}, and \textit{Mistral-7B-Instruct}. The same four mechanisms recur in all four models, showing that the organization we describe is a cross-family property of how these LLMs handle propositional reasoning rather than a feature of any single model.
%
Third, we introduce a prompt-order control intervention, \texttt{expr\_first}, that moves the queried expression before the factual assignments while preserving the underlying logical content, labels, and corruption semantics. This intervention tests whether staged computation follows the surface prompt layout or reflects a deeper model-internal preference: the same coarse early/middle/late template survives the reordering across all four models, and middle-layer fact-value causal density is in fact \emph{stronger} under \texttt{expr\_first} than under \texttt{facts\_first} in 7 out of 8 (model $\times$ order) cells. For readability, the main text figures use \texttt{facts\_first}; the full prompt-order analysis and cross-model tables are reported in Appendix~\ref{sec:appendix_prompt_order}, Appendix~\ref{sec:appendix_stage}, Appendix~\ref{sec:appendix_info_converge}, Appendix~\ref{appendix_heads}, and Appendix~\ref{sec:appendix_llama}.
...
\begin{figure*}[t]
    ...
    \vspace{-0.3em}
    \caption{\textbf{Overview of the four propositional-logic reasoning mechanisms identified in this work.} (a) Staged Computation: region-level MLP patching reveals layer-wise processing phases. (b) Information Transmission: per-token residual-stream patching shows that the largest per-token causal density in each layer band concentrates on a small set of semantically privileged boundary tokens. (c) Fact Retrospection: fact-value tokens are the only non-query category whose per-token causal density remains above the background floor into the middle and late bands, across all four model families we test. (d) Specialized Attention Heads: Splitting, Transmission, and Fact-Retrieval heads structurally implement the three macroscopic patterns, and their removal causes accuracy drops that the matched \textit{Random-Outside} control does not.}
    \vspace{-0.5em}
    \label{fig:overview}
\end{figure*}
...
A common three-level abstraction in mechanistic interpretability distinguishes features, circuits, and the algorithms they implement \citep{bereska2024mechanisticinterpretabilityaisafety}. Building on this view, we organize prior work on LLM reasoning along two complementary axes. 
A \textit{component-level} lens asks \textit{which} heads or layers implement a given sub-computation, while a \textit{strategy-level} lens asks \textit{how} those components are orchestrated into a network-wide reasoning pattern. 
As \citet{NEURIPS2024_47c7edad} observe, the specific attention heads realizing a functional component may shift across training and scale, yet the overarching algorithm they implement remains stable, motivating the need for the latter view. Our work contributes primarily on the strategy-level axis while drawing its primitives from the component-level literature reviewed below.
...
For reasoning specifically, \citet{kim2024reasoningcircuitslanguage} decompose syllogistic inference into modular circuits with premise-retrieval and conclusion-assembly components. Together, these studies provide fine-grained causal evidence at the level of individual heads and layers.

% \paragraph{Strategy-Level Reasoning Patterns.}
...
%
Beyond the CoT setting, prior component-level studies have shown that models route entity identity through binding ID vectors \citep{feng2024languagemodelsrepresent} and localize single-fact retrieval to specific MLP layers \citep{meng2023locatingeditingfactualassociations}. Our findings extend beyond these single-primitive operations: Transmission Heads route \textit{composed logical content} across token positions, and Fact Retrospection reveals that compositional reasoning relies on \textit{repeated} rather than \textit{one-shot} access to premises across network depth.
...
\paragraph{Mechanistic Studies of Propositional Logic.}
% Benchmarks such as PrOntoQA \citep{saparov2023languagemodelsgreedy}, ProofWriter \citep{tafjord2021proofwritergeneratingimplications}, and LogicBench \citep{parmar2024logicbench} evaluate LLM propositional reasoning behaviorally. They reveal systematic limitations: accuracy degrades with proof depth \citep{saparov2023languagemodelsgreedy, dziri2023faithfatelimitstransformers}, models struggle with proof planning when multiple valid inference steps exist \citep{saparov2023languagemodelsgreedy}, and they show particular difficulty with negated premises \citep{parmar2024logicbench}. These findings fuel the ongoing debate over whether LLMs implement genuine logical algorithms \citep{nanda2023progress} or merely exploit statistical shortcuts \citep{dziri2023faithfatelimitstransformers}. Behavioral evaluation alone, however, answers only \textit{whether} models succeed, not \textit{how} they complete the reasoning internally.
%
To the best of our knowledge, the only mechanistic study on propositional logic is \citet{aimpliesb}, which decomposes implication reasoning into four functional head types (locating, mover, processing, and decision heads) in \textit{Mistral-7B} and \textit{Gemma-2}, pinpointing the heads responsible for rule retrieval, information movement, and answer selection. Building on this head-level decomposition, we characterize network-wide computational patterns: how processing is temporally organized across layers, how information flows across token positions, and how facts are repeatedly retrieved throughout network depth. Combined with broader rule coverage and multi-hop scenarios, this pattern-level perspective reveals mechanisms that generalize across diverse logical structures, inference chain lengths, and model scales.
...
\section{PropLogic-MI: Task and Dataset}
\label{sec:proplogic_mi}
...
We formulate propositional logical reasoning as a next-token prediction problem over a \emph{logical instance} $I = (F, E, Q)$, where $F$ is a set of factual assignments, $E$ is a set of logical expressions, and $Q$ is the query. The gold answer $y \in \{\texttt{True}, \texttt{False}\}$ is fully determined by $I$ and is invariant to the order in which $F$, $E$, and $Q$ are presented to the model. To turn $I$ into a model input, we apply a \emph{surface linearization} $\pi$ that orders the components into a token sequence $x = \pi(I)$; throughout this paper we use two complementary linearizations: $\pi_{\mathrm{FF}}$ (\texttt{facts\_first}), which lists $F$ before $E$, and $\pi_{\mathrm{EF}}$ (\texttt{expr\_first}), which front-loads $E$ before $F$. Both linearizations preserve the same logical content $I$ and the same gold answer $y$, and both are evaluated for every model studied in this paper. The model $\mathcal{M}$ then predicts $y$ from $x$. To strictly probe implicit reasoning, we require the model to output the answer immediately after the query without \textit{Chain-of-Thought} (CoT) generation, confining all reasoning to internal hidden states.
...
To address this limitation, we introduce \textit{PropLogic-MI}, a highly controlled dataset designed specifically for circuit discovery. \textit{PropLogic-MI} focuses on atomic logical operations across 11 propositional logic rule categories (Table~\ref{tab:rule_coverage}), providing strictly aligned clean/corrupted pairs for one-hop and two-hop reasoning. Crucially, \textit{PropLogic-MI} is released in two prompt-order variants ($\pi_{\mathrm{FF}}$ and $\pi_{\mathrm{EF}}$, see \S\ref{sec:task}) that share rules, fact assignments, and gold labels and differ only in surface linearization, so the two variants together expose the same logical content under two complementary token orderings. This structural precision enables researchers to isolate the exact neural components responsible for logical transitions, shifting the focus from measuring performance to dissecting the underlying mechanism.
...
For each rule, we instantiate prompts by sampling truth values over the rule's atomic propositions and then randomly remapping the variable letters to fresh uppercase symbols drawn from a pool that excludes \texttt{T} and \texttt{F} (so the same logical row may surface as $A,B$ in one prompt and $G,W$ in another). Each prompt terminates with the fixed answer suffix \texttt{"Answer with one word only: True or False."}, which constrains the model to a binary truth-value output. A representative one-hop \textit{De Morgan} prompt under $\pi_{\mathrm{FF}}$ is therefore:
\begin{center}
\texttt{"G is False, W is False, ($\neg$G or $\neg$W) is. Answer with one word only: True or False."}
\end{center}
where the gold answer is \texttt{True}. Under $\pi_{\mathrm{EF}}$ the same row is linearized as \texttt{"($\neg$G or $\neg$W) is? G is False, W is False. Answer with one word only: True or False."}. The ground-truth answer is computed via symbolic evaluation; full corpus details are in Appendix~\ref{sec:appendix_dataset}.

\paragraph{Reasoning Depth}
We construct prompts under two complexity settings: (1) One-Hop Reasoning, where the query is directly derived from facts via a single rule, and (2) Two-Hop Reasoning, where an intermediate derived assignment $M = e_{1}$ is introduced inline (\textit{e.g.}, \texttt{"V is X or not X"}) and the final query then composes $M$ with a gate variable. Representative examples are provided in Table~\ref{tab:reasoning_depth}. Combined with the two surface linearizations $\pi_{\mathrm{FF}}$ and $\pi_{\mathrm{EF}}$ from \S\ref{sec:task}, every logical row therefore appears in four token-level variants (\{one-hop, two-hop\} $\times$ \{$\pi_{\mathrm{FF}}$, $\pi_{\mathrm{EF}}$\}).
...
\subsection{Sample Pools and Prompt-Order Variants}
\label{sec:sample_pools}

\paragraph{Sample Pools and Behavioral Baselines}
For each prompt-order variant of \textit{PropLogic-MI} we construct $20{,}000$ clean/corrupted pairs (\,$9{,}523$ one-hop, $10{,}477$ two-hop, nearly rule-balanced; see Appendix~\ref{sec:appendix_dataset}). Table~\ref{tab:dataset_baselines} reports clean accuracy and the \emph{dual-correct} rate (\textit{i.e.}, the fraction of prompts a model answers correctly on both the clean and the corrupted run) for all four models under both linearizations. The dual-correct subset is the per-model sample pool used by every mechanistic intervention in this paper. Llama-3.1-8B and Mistral-7B reach a markedly lower absolute accuracy than Qwen3, but as we show in \S\ref{sec:results} and Appendix~\ref{sec:appendix_llama}, the same four mechanisms recur in all four models on these dual-correct pools. Token-level patching is then run on balanced sub-samples of $n \approx 1{,}000$ prompts per (model, linearization) cell drawn from these pools, following the filtering pipeline described in Appendix~\ref{sec:appendix_setup}.
...
\toprule
& \multicolumn{2}{c}{$\pi_{\mathrm{FF}}$} & \multicolumn{2}{c}{$\pi_{\mathrm{EF}}$} & \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
\textbf{Model} & Clean & Dual & Clean & Dual & Pool (FF/EF) \\
\midrule
\textit{Qwen3-8B}            & 87.0 & 75.4 & 74.2 & 53.9 & 15{,}087 / 10{,}778 \\
\textit{Qwen3-14B}           & 87.4 & 76.6 & 74.4 & 52.8 & 15{,}310 / 10{,}562 \\
\textit{Llama-3.1-8B}        & 63.9 & 38.4 & 60.3 & 37.0 & \phantom{0}7{,}674 / \phantom{0}7{,}403 \\
\textit{Mistral-7B}          & 65.1 & 35.1 & 66.7 & 40.0 & \phantom{0}7{,}022 / \phantom{0}7{,}995 \\
\bottomrule
\end{tabular}
\caption{\textbf{Behavioral baselines on \textit{PropLogic-MI}} ($20{,}000$ prompts per linearization). Clean = clean-prompt accuracy (\%). Dual = fraction of prompts answered correctly on \emph{both} the clean and corrupted runs (\%). The dual-correct subset (right column) defines the per-model sample pool used for all mechanistic experiments. Llama-3.1-8B and Mistral-7B are markedly weaker in absolute accuracy than Qwen3, yet the same four mechanisms recur on their dual-correct pools (\S\ref{sec:results}, Appendix~\ref{sec:appendix_llama}). \textit{Mistral} is the only family whose dual-correct rate does not decrease under $\pi_{\mathrm{EF}}$; the reversal is driven entirely by one-hop prompts ($62.9 \to 75.7$\%) while two-hop drops ($67.1 \to 58.6$\%, see Appendix~\ref{sec:appendix_llama}).}
\label{tab:dataset_baselines}
\vspace{-0.6em}
\end{table}

\paragraph{Prompt-Order as a Sibling Linearization}
We refer to $\pi_{\mathrm{FF}}$ and $\pi_{\mathrm{EF}}$ as \emph{sibling linearizations}: two surface orderings of the same underlying logical content $I$ (§\ref{sec:task}) that differ only in which clause block (facts or expression) appears first. We adopt this term instead of \emph{prompt-order intervention} because the comparison is not a causal intervention in the activation-patching sense; it is a surface-form manipulation applied at the data-construction level.
We treat $\pi_{\mathrm{FF}}$ and $\pi_{\mathrm{EF}}$ symmetrically: both are run end-to-end through the full pipeline for every model. Because the two linearizations carry the same logical content, comparing them isolates effects that depend on \emph{surface token order} from effects that reflect \emph{model-internal computational preference}. Throughout the main text we display $\pi_{\mathrm{FF}}$ figures for readability and use $\pi_{\mathrm{EF}}$ as the matched control; the head-to-head comparisons across orderings are collected in Appendix~\ref{sec:appendix_performance} and Appendix~\ref{sec:appendix_prompt_order}.
...
\paragraph{Token Category Taxonomy}
To study the mechanistic roles of different prompt components (facts, expressions, and the query) at a finer granularity than the three-region partition of §\ref{sec:staged}, we partition each prompt's tokens into eight semantic categories produced by a deterministic, prompt-order-aware classifier (Table~\ref{tab:token_categories}; implementation in Appendix~\ref{sec:appendix_setup}). The eight categories are: \texttt{facts\_value}, \texttt{variable\_in\_facts}, \texttt{query\_token}, \texttt{variable\_in\_expr}, \texttt{operator}, \texttt{expr\_last}, \texttt{derived\_assignment}, and \texttt{others}. The same taxonomy is used across all four models and both prompt orders, so token-level results in \S\ref{sec:results} and Appendix~\ref{sec:appendix_llama} are directly comparable across cells.

\paragraph{Category-Level Aggregation}
\textit{Purpose.} To place causal effects from tokens with very different roles and cardinalities on a common per-token scale that is comparable across categories, layer bands, and model families.
\textit{Metric.} We aggregate per-token $\mathrm{dPD}$ values as the \textit{mean unsigned dPD},
\begin{equation}
  \overline{|\mathrm{dPD}|}_{c,b} = \frac{1}{|\mathcal{T}_{c,b}|} \sum_{t \in \mathcal{T}_{c,b}} |\mathrm{dPD}(t)|,
\end{equation}
where $\mathcal{T}_{c,b}$ is the set of tokens belonging to category $c$ (Table~\ref{tab:token_categories}) in layer band $b$ (\textit{early}, \textit{middle}, or \textit{late}). The absolute value avoids cancellation between positive and negative shifts within a category, and the mean normalizes by category cardinality so that categories with very different token counts (\textit{e.g.}, \texttt{facts\_value} typically $\sim$$2$ tokens per prompt versus \texttt{others} $\sim$$15$--$30$) remain directly comparable.
\textit{Role.} $\overline{|\mathrm{dPD}|}$ can thus be read as a \textit{per-token causal density}. This is the form used throughout §\ref{sec:results} and Appendix~\ref{sec:appendix_llama} to compare how strongly each category drives the prediction in each layer band, both within and across model families. We additionally define a \textit{staging sharpness ratio} $\rho_{\text{stage}} = \overline{|\mathrm{dPD}|}_{\texttt{facts\_value},\,\text{early}} / \overline{|\mathrm{dPD}|}_{\texttt{facts\_value},\,\text{middle}}$, which quantifies how quickly per-token fact-value density decays from the early to the middle band and thus serves as a family-level measure of staging sharpness independent of absolute density.
...
\midrule
\texttt{facts\_value}        & Truth-value tokens in the facts block (\texttt{True}, \texttt{False}). \\
\texttt{variable\_in\_facts} & Variable letters appearing in factual assignments outside the queried expression. \\
\texttt{query\_token}        & Query-region tokens: the query \texttt{is} anchor, the answer-suffix tokens, the final terminal token, and any chat-template tail. \\
\texttt{variable\_in\_expr}  & Variable letters that participate in the queried expression. \\
\texttt{operator}            & Logical operators inside an expression (\texttt{and}, \texttt{or}, \texttt{not}, \texttt{xor}, \ldots). \\
\texttt{expr\_last}          & The last non-punctuation token inside the queried expression span. \\
\texttt{derived\_assignment} & Non-punctuation tokens in the inline derived assignment $M = e_{1}$ for two-hop prompts. \\
\texttt{others}              & All remaining tokens (non-terminal punctuation, uncategorized expression tokens). \\
\bottomrule
\end{tabularx}
\caption{\textbf{Token category taxonomy.} Eight semantic categories used by the token-level analyses in \S\ref{sec:results} and Appendix~\ref{sec:appendix_llama}. The classifier is deterministic and prompt-order-aware, so the same taxonomy applies under both $\pi_{\mathrm{FF}}$ and $\pi_{\mathrm{EF}}$.}
...
\paragraph{Models}
We run the full mechanistic analysis on four LLMs drawn from three model families: \textit{Qwen3-8B}, \textit{Qwen3-14B}, \textit{Llama-3.1-8B-Instruct}, and \textit{Mistral-7B-Instruct}. For readability, the main-text figures show \textit{Qwen3-8B} as a representative case; the same pipeline is replicated on the other three models, with the corresponding figures and tables reported in Appendix~\ref{sec:appendix_stage}, Appendix~\ref{sec:appendix_info_converge}, Appendix~\ref{appendix_heads}, and Appendix~\ref{sec:appendix_llama}. All analyses use pretrained checkpoints in zero-shot settings without fine-tuning.

\paragraph{Evaluation}
All analyses apply activation patching and mean ablation across the 11 rule categories of \S\ref{sec:dataset} using $\text{PD}$ (Eq.~\ref{eq:logit_diff}) and $\text{dPD}$ (Eq.~\ref{eq:dld}), and aggregate per-token effects via the mean unsigned form $\overline{|\mathrm{dPD}|}_{c,b}$ defined above. Experiments run on NVIDIA H100 GPUs using TransformerLens~\citep{nanda2022transformerlens}; full implementation, the deterministic token classifier behind Table~\ref{tab:token_categories}, and the per-experiment hyperparameters are provided in Appendix~\ref{sec:appendix_setup}.
...
To answer it at the granularity of the input's logical structure, we use a \emph{region-level} causal probe, where ``region-level'' refers to the granularity of three semantic regions defined below, as opposed to the layer-wise or per-token granularities used elsewhere in this paper. Specifically, we partition each prompt into three semantically distinct regions: the \textit{Facts Region} (\textit{e.g.}, \texttt{A is True, B is False}), the \textit{Expression Region} (\textit{e.g.}, \texttt{($\neg$A or $\neg$B)}), and the \textit{Query Region}, which contains the query anchor together with the answer-format tail. For each region, we perform mean ablation on the MLP layer outputs across all layers in clean-prompt runs, quantifying the causal impact via 
$\text{dPD}$.  
...
We next investigate \textit{how information flows between computational stages}. Applying the residual-stream instantiation of our activation-patching framework (§\ref{sec:analytical_methods}) to a running example with clean prompt $x_{\text{clean}}$ = \texttt{A is True, B is False, ($\neg$A or $\neg$B) is} and corrupt prompt $x_{\text{corrupt}}$ = \texttt{A is True, B is True, ($\neg$A or $\neg$B) is}, we obtain the token-by-token causal traces in Figure~\ref{fig:logit_difference}. In Figure~\ref{fig:logit_difference}a, the patching effect is first concentrated at token position 6 (the truth value \texttt{True}) in early layers, shifts to token position 14 (the closing parenthesis \texttt{)}) in the middle, and converges at the query token in late layers; intra-segment tokens show negligible effects throughout. Information therefore aggregates at segment-terminal tokens before flowing to the final prediction position.

\paragraph{Pattern Quantification.} A second clean/corrupt pair (Figure~\ref{fig:logit_difference}b) corroborates this trajectory and identifies positions 2, 6, 14, and 15 as dominant convergence points. Aggregating these token-level effects by semantic category under the mean unsigned dPD aggregation (§\ref{sec:analytical_methods}) yields the same pattern, now directly comparable across categories of different cardinality (Figure~\ref{fig:info_converge_token_main}). In \textit{Qwen3-8B} under \texttt{facts\_first}, three different categories win the three layer bands: \texttt{facts\_value} dominates the early band with per-token causal density $\overline{|\mathrm{dPD}|} = 0.854$, \texttt{expr\_last} wins the middle band at $0.267$, and \texttt{query\_token} wins the late band at $0.088$; the matched \texttt{others} floor stays at $0.014$--$0.017$ throughout. Because the aggregation is normalized by the number of tokens per category, these numbers are comparable across categories of different cardinality and across model families, which is what lets §\ref{sec:retro} and Appendix~\ref{sec:appendix_llama} read the same axes on Llama and Mistral. Extended token-category results for \textit{Qwen3-14B}, two-hop reasoning, and the $\pi_{\mathrm{EF}}$ sibling linearization are reported in Appendix~\ref{sec:appendix_info_converge} and Appendix~\ref{sec:appendix_prompt_order}; Appendix~\ref{sec:appendix_extra_token_gallery} provides the retained refined token diagnostics.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{./figures_experiments/reports/token_analysis/8b_facts_first_all_hop_refined/refined_by_stage_mean.png}
    \caption{\textbf{Token-category information convergence in \textit{Qwen3-8B} under \texttt{facts\_first}.} Token-level residual-stream patching effects are aggregated by semantic category and layer band as \textit{mean unsigned dPD} (§\ref{sec:analytical_methods}), making per-token causal density directly comparable across categories of different cardinality. The early band is dominated by \texttt{facts\_value} ($\overline{|\mathrm{dPD}|}=0.854$), the middle band by \texttt{expr\_last} ($0.267$) with \texttt{facts\_value} still second at $0.209$, and the late band by \texttt{query\_token} ($0.088$); the \texttt{others} floor stays at $0.014$--$0.017$ across all bands. Concentrating the largest per-token density on a small set of semantically privileged categories, rather than spreading it uniformly across the prompt, is precisely what ``information convergence onto important tokens'' means under the mean unsigned aggregation.}
    \vspace{-0.5em}
...
\label{sec:retro}
The token-level aggregation from §\ref{sec:info} reveals a finding that is not predicted by a simple encode-then-forget view of the residual stream: \texttt{facts\_value} retains above-floor per-token causal density far beyond the early band. Under the mean unsigned aggregation, this phenomenon decomposes cleanly into two layer-band sub-findings that together define what we call \textit{Fact Retrospection}. We emphasize that the evidence here is causal density, not a direct measurement of information flow, and we therefore describe it in rank-order terms rather than agency terms throughout.

\paragraph{Middle-Band Dominance} In the middle layer band, \texttt{facts\_value} is the only non-query category whose per-token density is substantially above the \texttt{others} floor across all four model families and both prompt orders. Under \texttt{facts\_first}, the middle-band $\overline{|\mathrm{dPD}|}$ of \texttt{facts\_value} is $0.209$ (\textit{Qwen3-8B}), $0.336$ (\textit{Qwen3-14B}), $0.045$ (\textit{Llama-3.1-8B-Instruct}), and $0.092$ (\textit{Mistral-7B-Instruct}), against matched \texttt{others} floors of $0.014$, $0.011$, $0.011$, and $0.007$, i.e.\ floor ratios of $15\times$, $31\times$, $4\times$, and $13\times$. Under \texttt{expr\_first} the same ranking holds and is in fact stronger in 7 out of 8 (model $\times$ order) cells: middle-band \texttt{facts\_value} density rises to $0.390$, $0.575$, $0.072$, and $0.095$ for the four models, with matched floors of $0.008$, $0.004$, $0.010$, and $0.009$ (floor ratios $49\times$, $144\times$, $7\times$, and $11\times$). The middle-of-network layers therefore retain substantial per-token causal dependence on fact-value positions in every (model, order) cell.

\paragraph{Late-Band Rank-Order Residual} Late-band \texttt{facts\_value} density is numerically small in absolute terms ($0.004$--$0.029$ across the eight cells), but in every single cell it is the \textit{second-ranked} non-special category after \texttt{query\_token}, and it sits $2\times$--$29\times$ above the matched \texttt{others} floor (which decays to $0.000$--$0.002$ in the late band). Every other non-query category collapses essentially to floor, whereas \texttt{facts\_value} does not. This is not a claim that late layers ``return to'' the facts; it is a rank-order statement, namely that the fact-value positions are the only non-query category that carries a measurable residual causal signal into the late band of every model we tested.

\paragraph{Prompt-Order Sub-Finding} The middle-band comparison above reveals a further structural fact: reordering the prompt so that the query appears before the factual assignments (\texttt{expr\_first}) does not weaken middle-band fact-value density but \emph{increases} it in 7 of 8 cells, most strikingly for \textit{Qwen3-14B} ($0.336 \to 0.575$, a $1.7\times$ increase) and \textit{Qwen3-8B} ($0.209 \to 0.390$). Staging sharpness (§\ref{sec:analytical_methods}) is therefore prompt-order sensitive in a direction consistent with §\ref{sec:staged}: when the query is parsed first, the middle of the network carries more per-token causal dependence on fact-value positions rather than less.

\paragraph{Downstream Corroboration} The three token-level observations above come from residual-stream patching. Independently of this, the head-level ablation analysis in §\ref{sec:heads} shows that removing the top-$32$ Fact-Retrieval heads reduces held-out accuracy by $25.4$--$35.6$ points across all four validation settings, whereas a matched \textit{Random-Outside} control leaves accuracy near baseline. These two evidence streams are methodologically independent (token-position residual patching vs.\ head-level mean ablation), but they point in the same direction: fact-value positions remain a load-bearing component of the computation far beyond the early band. Full per-model breakdowns are provided in Appendix~\ref{sec:appendix_info_converge} and Appendix~\ref{sec:appendix_llama}.
...
\paragraph{Fact-Retrieval Heads} These heads route attention from the query token or expression tokens back toward truth-value tokens (\texttt{True}/\texttt{False}) (Figure~\ref{fig:facts_head}). Their concentration in middle-to-late layers (Figure~\ref{fig:counts}) provides the head-level counterpart to the rank-order residual reported in §\ref{sec:retro}: fact-value positions are the only non-query category whose causal density stays above floor into the middle and late bands, and these heads are the structural carriers of that residual at the attention level.
...
The same three-role template reproduces on \textit{Llama-3.1-8B-Instruct} and \textit{Mistral-7B-Instruct} (Appendix~\ref{sec:appendix_llama}), although the relative weight of each role differs from Qwen3: Llama and Mistral both show weaker Fact-Retrieval concentration and a heavier query-centered late-band contribution, consistent with the lower fact-value density reported in §\ref{sec:retro}. In other words, the head taxonomy generalizes to all four families, but \emph{how strongly} each role is expressed tracks the family-level density and sharpness axes identified earlier.

More importantly, this taxonomy is causally predictive rather than merely descriptive. Across all four Qwen3 validation settings (\textit{Qwen3-8B}/\textit{Qwen3-14B} $\times$ \texttt{facts\_first}/\texttt{expr\_first}), ablating the top-32 heads of a single role lowers accuracy by 20.4--39.6 points for Fact-Retrieval, 19.6--27.4 points for Transmission, and 10.0--19.6 points for Splitting. Ablating top-32 heads pooled across all three roles lowers accuracy by 25.4--35.6 points. In contrast, ablating 32 \textit{Random-Outside} heads leaves accuracy at or above baseline in three of the four settings, and causes only a 0.3-point drop in the remaining case. The strongest effect appears in \textit{Qwen3-8B} with \texttt{facts\_first} prompts: ablating the top-64 Fact-Retrieval heads reduces accuracy from 96.6\% to 50.6\%, whereas ablating 64 random outside heads still leaves 91.1\% accuracy. These interventions show that the extracted heads are functionally important for solving the task, and that the three roles capture complementary parts of the reasoning mechanism rather than superficial attention motifs. Appendix~\ref{appendix_heads} summarizes the robustness checks; Appendix~\ref{sec:appendix_full_accuracy} reports the complete Qwen3 Step~3 accuracy tables for all strategies, all $k$ values, and the pairwise / mixed-role combinations; and Appendix~\ref{sec:appendix_llama} shows how the same validation pipeline behaves in \textit{Llama-3.1-8B-Instruct} and \textit{Mistral-7B-Instruct}.

The same selectivity is visible in raw dPD space. Figure~\ref{fig:qwen3_dpd_shift_main} plots the main \textit{Qwen3-8B} \texttt{facts\_first} validation sweep as the mean intervention-induced shift in dPD, where more negative values indicate that the ablation pushes the model farther away from the correct answer. Fact-Retrieval remains the strongest single-role curve throughout the sweep: at $k=32$, its mean shift reaches $-0.668$, compared with $-0.456$ for Transmission, $-0.263$ for Splitting, and $+0.018$ for \textit{Random-Outside}. Even at $k=64$, the random control remains much weaker ($-0.089$) than the role-targeted ablations. The pairwise and mixed strategies interpolate between these behaviors rather than overturning the overall ordering, which supports the view that the discovered roles are complementary but unequally concentrated parts of the mechanism. Appendix~\ref{sec:appendix_extra_heads_gallery} extends the same raw dPD-shift diagnostic to both model scales and both prompt orders.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{./figures_experiments/reports/heads_analysis_qwen3_8b_facts_first/validation/plots/dpd_shift_curve.png}
\caption{\textbf{Raw Step~3 dPD-shift validation in \textit{Qwen3-8B} under \texttt{facts\_first}.} Each curve shows the mean change in dPD after ablating the top-$k$ heads for one strategy; more negative values indicate a more damaging intervention. Fact-Retrieval is the most harmful single-role ablation throughout the sweep ($-0.668$ at $k=32$; $-0.812$ at $k=64$), Transmission is second ($-0.456$ at $k=32$), Splitting is weakest ($-0.263$ at $k=32$), and \textit{Random-Outside} remains near zero through $k=32$ ($+0.018$) before becoming only mildly negative at $k=64$ ($-0.089$). This directional view complements Table~\ref{tab:qwen3_head_accuracy_main} by showing that the discovered roles move the model away from the correct answer in the expected causal direction rather than merely reducing held-out accuracy.}
\label{fig:qwen3_dpd_shift_main}
\end{figure}
...
Our results suggest that propositional logical reasoning in transformers is not implemented by a monolithic circuit but rather organized as a multi-phase computational strategy: early-layer encoding, middle-layer information routing, and late-layer decision making. This staged organization persists across 11 rule categories, two reasoning depths, and two model scales, which points to a general architectural property rather than a task-specific shortcut. This view complements the circuit-discovery line of work \citep{elhage2021mathematicalframeworktransformer, wang2022interpretabilitywildcircuitindirect}, which asks \textit{which} components are necessary for a task; here we instead ask \textit{how} the computation is organized along depth, and find a stage pattern that recurs across four model families rather than a single wiring diagram tied to one model.

A notable finding is Fact Retrospection: under the mean unsigned aggregation, fact-value positions are the only non-query category whose per-token causal density remains above the background floor into the middle and late layer bands, across four model families and both prompt orders. This cross-family rank-order residual is qualitatively different from the single-shot factual recall localized to early/middle MLPs in knowledge-editing studies \citep{meng2023locatingeditingfactualassociations}: the premises here do not disappear after being read, they continue to carry causal signal into the layers where the logical rule is composed. The cross-rule breakdowns make this more concrete. Accuracy and head-role strength vary smoothly across the 11 propositional rule categories, and Distributivity and De~Morgan are not only the hardest rules but also the settings where \textit{Fact-Retrieval} ablation produces the largest accuracy drops (Appendix~\ref{sec:appendix_performance}). Exactly the rules that require multiple re-accesses to the original truth values are the ones most damaged when we remove the heads that attend back to those positions, which provides indirect behavioral support for the Fact Retrospection interpretation. Whether the same residual pattern extends to other forms of compositional reasoning (\textit{e.g.}, arithmetic, spatial) remains an open question.

The staged computation we observe in the no-CoT setting also raises a question about chain-of-thought prompting: if models already implement internal staging, CoT may succeed not by introducing staged reasoning \textit{de novo}, but by aligning the model's internal computation with an explicit external structure. Testing whether CoT reshapes or merely amplifies the patterns we identify could clarify the mechanistic role of intermediate tokens.

\subsection{Rule-Type Variation}
The cross-rule breakdowns in Appendix~\ref{sec:appendix_performance} show that accuracy and head-role strength vary smoothly across the 11 propositional rule categories rather than splitting into a clean ``solved vs.\ unsolved'' partition. Commutative and Identity rules are the easiest for all four models; Distributivity and De~Morgan are the hardest and also the settings where Fact-Retrieval ablation produces the largest accuracy drops, which we read in §6.1 as behavioral support for Fact Retrospection. Two-hop reasoning amplifies these differences without introducing qualitatively new head roles, consistent with the view that multi-hop composition reuses the same mechanism at greater depth rather than instantiating a separate circuit.

\subsection{Head Taxonomy as a Descriptive Decomposition}
Beyond the three causally validated roles, two additional observations shape how the taxonomy should be read. First, we also see recurrent descriptive motifs that are \emph{not} part of the validated taxonomy: first-token sink heads, early entity-value binding heads, diagonal self-processing heads, and expression-focused heads in the late middle layers; we treat these as regularities in the attention maps rather than as additional causal claims. Second, some heads in the validated taxonomy exhibit multiple role signatures within a single attention map, combining boundary focus with local transmission, or transmission with fact retrieval. Appendix~\ref{sec:appendix_func_couple} gives qualitative examples. We therefore read Splitting, Transmission, and Fact-Retrieval as a descriptive decomposition of head behavior under this benchmark rather than a claim that each head performs only one operation, consistent with the parallel-circuit caveat noted in §Limitations.
...
In this paper, we present a mechanistic analysis of propositional logical reasoning across four LLMs (\textit{Qwen3-8B}, \textit{Qwen3-14B}, \textit{Llama-3.1-8B-Instruct}, and \textit{Mistral-7B-Instruct}), adopting a top-down analytical strategy
from
...
retains above-floor causal density on fact-value positions into middle and late layers,
...
These four phenomena form an interlocking system where each mechanism supports and validates the others. They recur across four model families, two model scales within Qwen3, 11 propositional rule categories, one-hop and two-hop reasoning depths, and two prompt orders (\texttt{facts\_first} and the \texttt{expr\_first} prompt-order control), suggesting fundamental computational strategies rather than task-specific shortcuts; the families differ along two independent axes of variation, density and sharpness, rather than along a single ``better staging'' ranking. By characterizing \textit{how} models reason rather than \textit{which components} are necessary, 
...
Third, our head-role discovery pipeline ranks heads by clean-prompt mean-ablation effect and clusters the top candidates by attention-pattern statistics. This procedure is causally validated at the group level by the comparison with the matched \textit{Random-Outside} control, which shows that the ranked-and-clustered heads are not merely a random high-importance set. However, group-level causal validation cannot rule out \textit{parallel circuits}: the possibility that several functionally redundant subsets of heads implement overlapping parts of the same computation, so that ablating one subset merely redirects mass to another. Our top-$k$ sweeps in §\ref{sec:heads} and Appendix~\ref{sec:appendix_extra_heads_gallery} are consistent with this concern (accuracy drops keep growing as $k$ increases), and we therefore treat the taxonomy as a descriptive decomposition rather than a uniqueness claim. We have also not performed individual-head targeted ablation within each role cluster; such within-cluster necessity tests would further sharpen the causal reading beyond the group-level Random-Outside comparison.

Fourth, our analysis is restricted to the implicit (no-CoT) regime in which the model must answer immediately after the query. This regime is methodologically convenient because it forces all reasoning into the residual stream, but it also means our claims should not be read as statements about chain-of-thought reasoning, where intermediate tokens change the computational graph.

Fifth, the late-band density of fact-value positions is numerically small in absolute terms ($\overline{|\mathrm{dPD}|}$ ranging from $0.004$ to $0.029$), and our late-band claim is a \textit{rank-order residual} claim (fact-value is the only non-query category above the background floor in every cell) rather than an agency claim. We have not performed matched clean-vs-incorrect failure-case patching on the late band, and we therefore defer the sharper question of whether this residual is load-bearing for the subset of prompts the model gets wrong to future work.

Finally, while we interpret our findings as evidence for structured computation, we cannot definitively rule out that these patterns reflect sophisticated statistical shortcuts that merely approximate genuine logical algorithms.
    ...
    \item[\ref{sec:appendix_results}] \hyperref[sec:appendix_results]{More Results}
	    \begin{itemize}
	        ...
	        \item[\ref{sec:appendix_info_converge}] \hyperref[sec:appendix_info_converge]{Information Transmission}
		        ...
		        \item[\ref{sec:appendix_llama}] \hyperref[sec:appendix_llama]{Cross-Model Extensions: Llama-3.1-8B-Instruct and Mistral-7B-Instruct}
...
\bottomrule
\end{tabularx}
\caption{\textbf{Rule coverage.} \textit{PropLogic-MI} systematically covers 11 fundamental rule categories, each shown with its formal Boolean-algebra definition and an illustrative one-hop prompt template. The example templates use the placeholder letters \{\texttt{A,B,C}\} with truth-value abbreviations \texttt{T}/\texttt{F} for compactness; in the actual corpus, the variable letters are randomly remapped per prompt to fresh uppercase symbols drawn from a pool that excludes \texttt{T} and \texttt{F}, and every prompt terminates with the fixed answer suffix \texttt{"Answer with one word only: True or False."} (Appendix~\ref{sec:appendix_dataset}).}
\vspace{-1.0em}
\label{tab:rule_coverage}
\end{table*}
...
\bottomrule
\end{tabularx}
\caption{\textbf{Probing prompts across reasoning depths.} We probe the model with queries requiring direct rule application (One-Hop) versus those necessitating an intermediate derived assignment $M = e_{1}$ that is written inline in the prompt (Two-Hop). The templates above use the compact placeholder letters \{\texttt{A,B,C,D}\} for readability; in the actual corpus, variables are randomly remapped per prompt to fresh uppercase symbols (excluding \texttt{T}/\texttt{F}), and every prompt terminates with the fixed answer suffix \texttt{"Answer with one word only: True or False."} The same logical row is realized under both linearizations $\pi_{\mathrm{FF}}$ and $\pi_{\mathrm{EF}}$ (\S\ref{sec:task}, Appendix~\ref{sec:appendix_dataset}).}
...
\begin{figure*}[t]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{\textit{Mistral-7B-Instruct}, \texttt{facts\_first}}
...
\begin{figure*}[t]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{\textit{Mistral-7B-Instruct}, \texttt{expr\_first}}
...
%      \end{subfigure}
%
%      \caption{\textbf{Mistral-7B-Instruct: band-level overviews for all three region-patching analyses under both prompt orders.} Rows correspond to MLP, attention, and joint
...
For completeness, we collect the full Step 3 accuracy tables at the end of the appendix. This keeps the main analysis sections readable while preserving the full strategy-by-$k$ results for \textit{Qwen3}, \textit{Llama-3.1-8B-Instruct}, and \textit{Mistral-7B-Instruct}.

\paragraph{Reading guide.}
For quick lookup, the four \textit{Qwen3} tables appear in Tables~\ref{tab:qwen3_8b_ff_full_accuracy}--\ref{tab:qwen3_14b_ef_full_accuracy}: \textit{Qwen3-8B} under \texttt{facts\_first} and \texttt{expr\_first} is given in Tables~\ref{tab:qwen3_8b_ff_full_accuracy} and~\ref{tab:qwen3_8b_ef_full_accuracy}, while \textit{Qwen3-14B} is given in Tables~\ref{tab:qwen3_14b_ff_full_accuracy} and~\ref{tab:qwen3_14b_ef_full_accuracy}. The corresponding \textit{Llama-3.1-8B-Instruct} results are reported in Tables~\ref{tab:llama_ff_full_accuracy} and~\ref{tab:llama_ef_full_accuracy}, and the \textit{Mistral-7B-Instruct} results are reported in Tables~\ref{tab:mistral_ff_full_accuracy} and~\ref{tab:mistral_ef_full_accuracy}. Within each family, the \texttt{facts\_first} table appears before the \texttt{expr\_first} table so the prompt-order comparison can be read left-to-right at the model level.

\paragraph{Connection to the main text.}
These tables are best read together with Section~\ref{sec:heads} and the compact Qwen3 summary in Table~\ref{tab:qwen3_head_accuracy_main}. The main text establishes the headline result, namely that role-targeted ablations are much more damaging than \textit{Random-Outside}. The full appendix tables refine that claim in two ways: first, they show how rapidly each role becomes harmful as the ablation budget $k$ increases; second, they show whether combining roles produces additional damage or instead dilutes the intervention by spreading a fixed budget across less critical heads. This makes the appendix useful not only as a completeness check, but also as a more detailed causal probe of how concentrated each model's head-level mechanism really is.

\paragraph{Interpretation.}
Taken together, these full tables sharpen the causal contrast between the three model families. In \textit{Qwen3}, role-targeted ablations worsen accuracy in a mostly monotonic way as $k$ grows, with Fact-Retrieval usually producing the largest drop and \textit{Random-Outside} remaining near baseline except at the largest ablation budgets. This is the cleanest evidence for selective head-level specialization: the discovered roles are not only useful labels, but also isolate a comparatively compact set of causally important heads. \textit{Llama-3.1-8B-Instruct} shows a different regime. Many strategy variants collapse to a similar accuracy floor after relatively small $k$, while \textit{Random-Outside} often stays near or slightly above baseline, which suggests that the recovered roles are much less cleanly separated from the rest of the head pool. \textit{Mistral-7B-Instruct} lies between these extremes: the discovered roles are highly consequential, but the random control is also substantially destructive at moderate and large $k$, indicating a denser and more globally sensitive head set. Across prompt orders, \texttt{expr\_first} mainly reallocates which heads become important rather than removing the role structure altogether, but the full tables show that this reallocation is handled most cleanly by \textit{Qwen3} and least cleanly by \textit{Llama}.

\paragraph{Role-combination effects.}
The combination rows are especially informative. In \textit{Qwen3}, combining roles is \emph{not} automatically more harmful at a fixed $k$. Under \textit{Qwen3-8B} \texttt{facts\_first}, ablating Fact-Retrieval alone at $k=32$ already reduces accuracy to $57.0\%$, essentially matching Fact-Retrieval+Splitting ($57.6\%$) and Fact-Retrieval+Transmission ($57.4\%$), while \textit{Mixed-All} is actually weaker ($62.0\%$). This suggests that in that setting the dominant causal mass is already concentrated inside the Fact-Retrieval set, so distributing the same budget across multiple roles partly dilutes the intervention. By contrast, in \textit{Qwen3-14B}, especially under \texttt{expr\_first}, pairwise and mixed ablations become more revealing of inter-role dependence: at $k=32$, Fact-Retrieval+Transmission drops accuracy to $67.6\%$, clearly below Fact-Retrieval alone ($77.2\%$) or Transmission alone ($76.6\%$), and \textit{Mixed-All} falls further to $62.0\%$. This is consistent with the main-text picture that mid-layer transmission and late-layer fact-value causal density (§\ref{sec:retro}) behave as a coupled pathway rather than as fully separable modules.

The same comparison also clarifies the status of Splitting Heads. Pairs that exclude Fact-Retrieval, such as Splitting+Transmission, are usually less harmful than Fact-Retrieval+Transmission at the same $k$, which suggests that boundary-setting is mechanistically important but often less singularly bottlenecked than the later routing-and-retrieval pathway. The cross-model tables reinforce this reading. In \textit{Llama-3.1-8B-Instruct}, most single-role, pairwise, and mixed strategies quickly collapse to nearly the same floor ($55.2\%$ under \texttt{facts\_first}; $58.4\%$ under \texttt{expr\_first}), so combinations reveal little additional structure. In \textit{Mistral-7B-Instruct}, combinations accelerate the collapse at small $k$, but by moderate budgets many role-targeted strategies again converge to the same low-$40\%$ floor. The appendix therefore strengthens a broader conclusion already implicit in the main text: \textit{Qwen3} exhibits the cleanest selective division of labor, whereas \textit{Llama} is more diffuse and \textit{Mistral} is more globally fragile.
...
\caption{\textbf{Full Step 3 accuracy table for \textit{Mistral-7B-Instruct} with \texttt{facts\_first} prompts.} Base held-out accuracy is $99.4\%$. All values are post-ablation accuracies and accuracy drops in percentage points. Compared with the Qwen3 tables, Mistral exhibits much stronger overall sensitivity: role-targeted ablations collapse quickly toward the low-40\% range, and even the Random-Outside control causes a substantial degradation.}
...
\caption{\textbf{Full Step 3 accuracy table for \textit{Mistral-7B-Instruct} with \texttt{expr\_first} prompts.} Base held-out accuracy is $99.8\%$. All values are post-ablation accuracies and accuracy drops in percentage points. The same qualitative pattern remains visible as in the \texttt{facts\_first} table: the discovered roles are highly consequential, but the random-outside control is also substantially destructive, indicating a sensitive yet less cleanly isolated head set.}
