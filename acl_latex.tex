...
\begin{figure*}[t]
  ...
  profile in one-hop to a more clearly middle-to-late bridge in two-hop (middle/late BCR $0.329/0.093 \rightarrow 0.487/0.140$). Appendix~\ref{sec:appendix_stage} extends the same
  ...
  band-composition galleries and stacked BCR summaries.}
    \label{fig:stage}
    \vspace{-0.5em}
  \end{figure*}
...
\begin{figure}[t]
    ...
    Error bars denote standard error of the mean (SEM). Appendix~\ref{sec:appendix_extra_token_gallery} provides the omitted raw heatmaps and refined token-diagnostic galleries.}
    \vspace{-0.5em}
...
Intuitively, \textbf{BMI} answers ``how strong is this region on average in this band?'', \textbf{BCR} answers ``where in depth is this region's total causal mass concentrated?'', and \textbf{SBI} answers ``is that effect directionally harmful or helpful for the correct answer?'' In practice, high BMI and high BCR in an early Facts cell indicate a sharply localized early computation stage, whereas a broad middle/late Query BCR indicates that final integration is distributed across the back half of the network rather than concentrated at a single layer.
...
\paragraph{Roadmap.}
This appendix proceeds from compact summaries to increasingly specialized diagnostics. We begin with the behavioral baseline and the prompt-order intervention, then extend the main-text analyses at the region, token, and head levels, and finally collect cross-model comparisons and complete validation tables. The large figure galleries included later should therefore be read as expansions of the corresponding main-text claims rather than as disconnected result families.

\paragraph{How to read the recurring figures.}
Several visual formats reappear throughout the appendix. In the band-metric panels, \textbf{BMI} asks how strong a region's average effect is within a depth band, \textbf{BCR} asks what fraction of that region's \emph{total} causal mass falls in that band, and \textbf{SBI} restores sign: strongly negative values indicate that mean-ablation typically lowers support for the correct answer, while positive values indicate that the replacement is directionally helpful. In the stacked-BCR charts, each region sums to $1$, so the plot should be read as a \emph{where-in-depth} decomposition rather than as a raw magnitude comparison across regions. In the token plots, larger unsigned values mean a token class carries more answer-relevant information, while signed variants distinguish large-but-canceling effects from directionally consistent ones. In the head-validation curves, steeper upward absolute-ratio curves or more negative raw-dPD curves indicate that ablating the selected heads is more harmful.
...
\paragraph{How to read the band panels.}
Figures in this subsection are best read row-wise by hop depth and then across the BMI/BCR/SBI columns. Large BMI indicates a strong average ablation effect in that band; large BCR indicates that the band accounts for a large share of that region's total effect; and strongly negative SBI indicates that replacing the region with its mean consistently hurts correct-answer support. The most informative cells are therefore usually the ones that are both high in BCR and directionally negative in SBI, because they identify not just large magnitude, but \emph{where} the model is systematically relying on that region.

\paragraph{Staged computation under prompt reordering.}
The MLP results already show a systematic prompt-order shift rather than a collapse of the stage structure. Figure~\ref{fig:expr_first_stage_panel} summarizes the full band-level comparison, while Figure~\ref{fig:query_region_prompt_order} isolates the query-region curves. In one-hop \textit{Qwen3-8B}, the early-band contribution ratio (BCR) of the Expression Region increases from $0.69$ under \texttt{facts\_first} to $0.77$ under \texttt{expr\_first}, while the Facts Region early-band BCR falls from $0.86$ to $0.69$. The same pattern appears in one-hop \textit{Qwen3-14B}, where the Expression Region early-band BCR rises from $0.68$ to $0.84$ and the Facts Region early-band BCR falls from $0.82$ to $0.60$. Meanwhile, the Query Region remains a late-stage aggregation site, with its late-band BCR increasing from $0.13$ to $0.19$ in one-hop \textit{Qwen3-8B}. This is consistent with a prompt-sensitive redistribution of computational load: placing the queried expression first pulls expression processing earlier, but it does not remove the late-stage answer integration step.
...
The interpretive distinction is straightforward. If a branch mainly changes \textbf{BMI}, it changes the \emph{strength} of the intervention effect. If it mainly changes \textbf{BCR}, it changes \emph{where in depth} the model spends that effect. The attention branch is especially informative in this regard because it often leaves the coarse region order intact while redistributing more mass into middle- and late-band routing.
...
\begin{figure*}[t]
  \includegraphics[width=\textwidth]{./figures/info-convergence-multi-corrupted.png}
...
The token-analysis branch is now complete. Figure~\ref{fig:token_facts_first_cross_model} collects the current \texttt{facts\_first} refined token-analysis outputs for all four models under a shared layout, with one subfigure per model and prompt template; Appendix~\ref{sec:appendix_extra_token_gallery} adds the raw and signed token diagnostics that make the same inter-family contrast more explicit. In the refined \texttt{facts\_first} token analysis, the dominant Llama categories are still \texttt{query\_token} and \texttt{facts\_value}, but their total contribution is much weaker than in Qwen3-8B: $1.63$ and $0.72$ in Llama, versus $2.22$ and $2.47$ in Qwen3-8B. This suggests that the same information-transmission motif is present, but less sharply concentrated.
...
At the region level, Mistral still shows a coarse staged profile in the MLP branch, shown in Figure~\ref{fig:mistral_mlp_panel}. Figure~\ref{fig:mistral_band_overview} extends this to all three branches, and Appendix~\ref{sec:appendix_extra_region_gallery} adds the stacked-BCR and full-gallery views. The attention-region outputs indicate an even stronger late Query Region than the MLP branch: on one-hop \texttt{facts\_first}, the Query Region late-band BCR rises from $0.15$ in MLP ablation to $0.27$ in attention ablation. The joint branch stays more stage-like than pure attention, but still retains a broader query-facing tail than Qwen3. This suggests that Mistral may rely more heavily on late routing and integration than the main Qwen3 models.
...
Mistral also yields the same three head roles, with $314$ classified heads under \texttt{facts\_first} (Fact-Retrieval $118$, Splitting $99$, Transmission $97$) and $307$ under \texttt{expr\_first} (Fact-Retrieval $123$, Splitting $109$, Transmission $75$). Figure~\ref{fig:mistral_heads_panel} collects the corresponding role distributions and validation curves. The Step 3 profile is strikingly sensitive. Base held-out accuracy is $99.4\%$ / $99.8\%$ for \texttt{facts\_first} / \texttt{expr\_first}, but $k=32$ role-targeted ablations reduce this to $41.2$--$41.4\%$ and $43.2$--$44.8\%$, respectively. Unlike Qwen3, however, the Random-Outside control is not benign: it still drops accuracy to $88.4\%$ and $85.3\%$. We interpret this as evidence that Mistral's performant head subset is highly sensitive overall, but less cleanly isolated from the surrounding pool than in Qwen3. The complete Mistral Step 3 accuracy tables are reported in Tables~\ref{tab:mistral_ff_full_accuracy} and~\ref{tab:mistral_ef_full_accuracy}.
...
We analyze cases where the model produces incorrect outputs despite exhibiting the same functional head profiles observed in successful reasoning. Figure~\ref{fig:corporate_heads} provides a schematic recap of the three-role picture used to organize this discussion. The analysis reveals that failures are not due to a breakdown in head specialization, but rather to limitations in downstream integration.
...
A recurring background phenomenon in the Qwen3 attention maps is a \textit{first-token attention sink}: many heads allocate a nontrivial fraction of their mass to position $0$, even when their main task-relevant structure is located elsewhere. In our visual inspections, this effect becomes especially common from roughly the middle of the network onward (around Layer~10 and above), where a visible first-column stripe often coexists with the more semantically meaningful patterns discussed in the main text. This sink should therefore be read as a global positional anchor rather than as a separate logical operation. In some heads it remains a weak background bias; in others it dominates the entire map and produces the idle-head pattern described next. Figure~\ref{fig:other_heads}(a) shows the strongest form of this behavior, while Figure~\ref{fig:other_heads}(g) illustrates a mixed case in which expression-focused attention coexists with a clear first-token sink.
...
These additional patterns complement rather than contradict our main findings. The attention-sink phenomenon suggests that part of the attention budget is reserved for a global positional anchor, upon which more task-specific patterns are superimposed. Idle Heads may therefore represent the limiting case where this sink-like behavior dominates almost completely. Information Binding Heads provide the early-layer foundation that enables later Fact-Retrieval. Self-Processing Heads may contribute to the residual stream's role in information maintenance. Expression Processing Heads bridge the gap between Information Transmission and final prediction, aligning with the Staged Computation framework. While these patterns have not been causally validated through patching experiments, they suggest functional diversity beyond the three primary head types and warrant further investigation.
