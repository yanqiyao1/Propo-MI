...
Understanding how Large Language Models (LLMs) perform logical reasoning internally remains a fundamental challenge. Prior mechanistic studies often focus on task-specific circuits, leaving open the broader question of what computational strategies LLMs use for propositional reasoning. We address this gap with a causal analysis on \textit{PropLogic-MI}, a controlled benchmark spanning 11 propositional logic rule categories across one-hop and two-hop tasks. Our primary analysis centers on \textit{Qwen3} (8B and 14B), with cross-model extensions to \textit{Llama-3.1-8B-Instruct} and \textit{Mistral-7B-Instruct-v0.1}. Rather than asking only which components are necessary, we ask how the computation is organized. Across region-level ablations, residual-stream patching, and attention-head validation, we identify four interlocking mechanisms: Staged Computation (layer-wise processing phases), Information Transmission (aggregation at semantically privileged boundary tokens), Fact Retrospection (persistent re-access of source facts), and Specialized Attention Heads (functionally distinct head roles). These patterns remain visible across Qwen3 scales, prompt orders, rule types, and reasoning depths. The cross-model results show that the same broad motifs recur in Llama and Mistral, but with weaker head-role selectivity and heavier query-centered computation than in Qwen3. Together, these findings provide mechanistic evidence that propositional reasoning in LLMs is structured rather than monolithic, while also revealing substantial family-level differences in how cleanly that structure is realized.
...
The appendix also collects the supporting scale, token, and head extensions that back these generalization claims, especially Appendix~\ref{sec:appendix_stage}, Appendix~\ref{sec:appendix_info_converge}, Appendix~\ref{appendix_heads}, and the cross-model summary in Appendix~\ref{sec:appendix_llama}.
