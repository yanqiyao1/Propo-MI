...
The remaining Qwen3 figures fill three specific gaps: the omitted two-hop attention gallery, the omitted two-hop joint attention+MLP gallery, and the retained facts-first two-hop MLP gallery. Together they show that increased reasoning depth broadens the Query Region without changing the overall Facts $\rightarrow$ Expression $\rightarrow$ Query order. We also retain the facts-first absolute-ratio head-validation curves in Figure~\ref{fig:qwen_heads_abs_ratio_facts}, which confirm that the same role ordering visible in the raw dPD-shift plots remains visible under the normalized validation metric.
