...
ONE_HOP_INPUT="${ONE_HOP_INPUT:-dataset/attn_mlp_8b_one_hop.jsonl}"
TWO_HOP_INPUT="${TWO_HOP_INPUT:-dataset/attn_mlp_8b_two_hop.jsonl}"
SPLIT_SUMMARY="${SPLIT_SUMMARY:-reports/attn_mlp_analysis/8b_dual_correct_split.json}"
OUTPUT_ROOT="${OUTPUT_ROOT:-reports/attn_mlp_analysis}"
...
usage() {
  cat <<USAGE
Usage: bash scripts/ATTN_MLP_region_patching.sh [options]
...
run_one() {
  ...
  summary_path="${output_dir}/attn_mlp_${region}_summary.json"
  scores_csv="${output_dir}/attn_mlp_${region}_scores.csv"
  ...
  python -m src.attn_mlp_analysis.attn_mlp_region_ablation \
    --model_id "${MODEL_ID}" \
...
plot_one() {
  ...
  python -m src.attn_mlp_analysis.plot_region_scores \
    --scores_csv "${output_dir}/attn_mlp_${region}_scores.csv" \
    --output_png "${output_dir}/attn_mlp_${region}_scores.png"
}
...
for region in regions:
    summary_path = base_dir / region / f"attn_mlp_{region}_summary.json"
