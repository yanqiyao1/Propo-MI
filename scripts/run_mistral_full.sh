...
if [[ "${RUN_HEADS}" == "1" ]]; then
  bash scripts/mistral_head_analysis.sh
fi

if [[ "${RUN_MLP}" == "1" ]]; then
  bash scripts/mistral_mlp_patching.sh
fi

if [[ "${RUN_TOKEN}" == "1" ]]; then
  bash scripts/mistral_token_analysis.sh
fi

echo "[done] mistral full pipeline completed"
