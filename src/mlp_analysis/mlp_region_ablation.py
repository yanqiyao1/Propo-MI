...
from .plotting import plot_bar as _plot_bar
from .result_records import build_sample_detail_record, build_skipped_sample_record, summarize_sample_layers
...
def main() -> None:
    parser = argparse.ArgumentParser(description="MLP region ablation with layout-aware region parsing")
    ...
    parser.add_argument("--max_samples", type=int, default=0, help="Balanced per-rule sample budget; 0 means all.")
    ...
    logger = setup_file_logger(__name__, resolve_log_path(output_dir=args.output_dir, filename="mlp_region_ablation.log"))

    rows = read_jsonl(args.input)
    if args.max_samples > 0:
        rows = balanced_sample_by_rule(rows, max_samples=args.max_samples)
    if len(rows) == 0:
        raise ValueError("No rows available for mlp region ablation")
    ...
    layer_scores_path = args.output_dir / f"mlp_{args.region_mode}_sample_layer_scores.csv"
    sample_details_path = args.output_dir / f"mlp_{args.region_mode}_sample_details.jsonl"
    skipped_samples_path = args.output_dir / f"mlp_{args.region_mode}_skipped_samples.jsonl"
    ...
    try:
        with torch.no_grad():
            progress = make_tqdm(rows, total=len(rows), desc=f"mlp-{args.region_mode}", leave=True, disable=len(rows) <= 1)
            for idx, row in enumerate(progress, start=1):
                ...
                for layer in range(n_layers):
                    ...
                    def hook_fn(mlp_out, hook, region_idxs=tuple(region_indices)):
                        del hook
                ...
                if args.progress_every > 0 and (idx % args.progress_every == 0 or idx == len(rows)):
                    ...
                    log_event(logger, {"stage": "mlp_region_progress", "region_mode": args.region_mode, "done": idx, "total": len(rows)})
    finally:
    ...
    csv_path = args.output_dir / f"mlp_{args.region_mode}_scores.csv"
    ...
    summary = {
        ...
        "abs_score_definition": "abs_dpd = |dPD| = |patched_margin - base_margin|",
        "band_metrics_definition": "BMI/BCR/SBI are kept unchanged from the previous normalized-score pipeline.",
        "csv": str(csv_path),
        "band_metrics": band_metrics,
        "plot_png": str(args.output_dir / f"mlp_{args.region_mode}_scores.png"),
        "abs_plot_png": str(args.output_dir / f"mlp_{args.region_mode}_abs_dpd_scores.png"),
    ...
    band_metrics_path = args.output_dir / f"mlp_{args.region_mode}_band_metrics.json"
    ...
    summary_path = args.output_dir / f"mlp_{args.region_mode}_summary.json"
    summary_path.write_text(json.dumps(summary, ensure_ascii=True, indent=2) + "\n", encoding="utf-8")

    plot_path = args.output_dir / f"mlp_{args.region_mode}_scores.png"
    abs_plot_path = args.output_dir / f"mlp_{args.region_mode}_abs_dpd_scores.png"
    if args.save_plots:
        _plot_bar(
        ...
        _plot_bar(
            output_png=abs_plot_path,
            layers=list(range(n_layers)),
            scores=[abs(value) for value in dpds],
            ylabel="|dPD|",
        )
